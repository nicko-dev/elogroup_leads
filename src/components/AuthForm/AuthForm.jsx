...
import { login, singUp } from '../../storeConfig/slices/authSlice';
            ...
            <Paper elevation={3} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 2 }}>
                ...
                <form style={{ width: '100%' }} onSubmit={handleSubmit(onSubmit)} autoComplete='off'>
                    <Grid container spacing={2} justifyContent='center' sx={{ paddingY: 3 }}>
                        {!isSignUp && (
                            <>
                                <Input name='firstName' label='Nome  *' type='text' autoFocus={isSignUp ? false : true} half register={register} />
                                <Input name='lastName' label='Sobrenome *' type='text' half register={register} />
                            </>
                        )}
                        <Input name='email' label='Email *' type='text' autoFocus={!isSignUp ? false : true} register={register} />
                        <Input name='password' label='Password *' register={register} type={showPassword ? 'text' : 'password'} handleShowPassword={handleShowPassword} />
                        {!isSignUp && (
                            <>
                                <Input name='confirmPassword' label='Corfirmação de Password *' register={register} type={showConfirmPassword ? 'text' : 'password'} handleShowPassword={handleShowConfirmPassword} />
                            </>
                        )}
                        <Grid item >
